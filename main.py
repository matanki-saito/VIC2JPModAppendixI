...
def replace_parameters(replace_parameter_map,
    ...
    for source_file_path in pathlib.Path(source_dir_path).glob('**/*.csv'):
...
def main():
    ...
    mod_pack_file_path = pack_mod(
        out_file_path=_(".", "out", "vic2_ap1_mod"),
    ...
    cdn_url = upload_mod_to_google_drive(
        ...
        name=dt.now().strftime('%Y-%m-%d_%H-%M-%S-{}.zip'.format("vic2-ap1")),
    ...
    # git pushするために上書きする
    for source_file_path in pathlib.Path(localisation_dir_path).glob('**/*.csv'):
